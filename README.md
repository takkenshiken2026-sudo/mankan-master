...
## AdSense 審査モード（`auditMode`）

`site-config.json` の `"auditMode": true` の間だけ、**公式過去問の個別ページ（`q/past/**`）を `noindex` にして検索インデックス・サイトマップから外します**。広告審査中に「コンテンツの複製」とみなされるリスクを一時的に避けるための設定です。実践演習・用語・記事・過去問一覧ハブ（`q/index.html`）は通常どおりインデックスされます。

**AdSense の承認後は `auditMode` を `false` に戻し、再ビルド・再デプロイ**すれば過去問ページが元どおりインデックスされます（`main` へ push すると自動デプロイ）。
