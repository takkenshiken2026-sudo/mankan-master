#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ガイド CSV / batch から学習運用テンプレ jargon（7行表 等）を除去する。

field-* 分野別記事は対象外（fix batch で個別対応済み）。
study-plan-* は「分野別正答率表」表記を正とする。

  python3 tools/strip_study_schedule_jargon.py --dry-run
  python3 tools/strip_study_schedule_jargon.py
  python3 tools/strip_study_schedule_jargon.py --batches
"""
...
BATCH_GLOB = "mankan_rewrite_batch*.py"

# 長いパターンから順に適用
JARGON_REPLACEMENTS: list[tuple[str, str]] = [
    (r"7行表「管」行", "分野別正答率表の管理組合行"),
    (r"7行表「区」行", "分野別正答率表の区分所有法行"),
    (r"7行表「建」行", "分野別正答率表の建物・設備行"),
    (r"管7行表", "管理組合分野の正答率記録"),
    (r"区7行表", "区分所有法分野の正答率記録"),
    (r"同7行表", "同じ分野別正答率表"),
    (r"7行表2週1行更新", "分野別正答率の2週ごと1行更新"),
    (r"7行表1行更新", "分野別正答率1行の更新"),
    (r"7行表1行記録", "分野別正答率1行の記録"),
    (r"7行表1行を", "分野別正答率1行を"),
    (r"→7行表1行", "→分野別正答率1行"),
    (r"7行表1行", "分野別正答率1行"),
    (r"7行表70％", "分野別正答率表での70％配分"),
    (r"7行表48％", "分野別正答率表での48％ライン"),
    (r"7行表で修正", "分野別正答率表で修正"),
    (r"7行表で管理", "分野別正答率表で管理"),
    (r"7行表で", "分野別正答率表で"),
    (r"7行表と", "分野別正答率表と"),
...
def patch_batch_files(*, dry_run: bool = False) -> tuple[int, int]:
    file_count = 0
    hit_count = 0
    for path in sorted((ROOT / "tools").glob(BATCH_GLOB)):
...
def main() -> int:
    ...
    parser.add_argument("--batches", action="store_true", help="batch 正本も更新")
    ...
    if args.batches and not args.dry_run:
        pass  # batches already patched above unless csv-only
