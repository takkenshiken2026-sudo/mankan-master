/**
 * 比較・整理表一覧 terms/compare/index.html
 * 再生成: python3 tools/build_compare_pages.py
 */
(() => {
  'use strict';
  ...
  const dataEl = document.getElementById('compare-index-data');
  const ITEMS = dataEl ? JSON.parse(dataEl.textContent || '[]') : [];

  const q = document.getElementById('compare-idx-q');
  const chips = $$('.terms-idx-chip[data-cat]');
  const hit = document.getElementById('compare-idx-hit');
  const empty = document.getElementById('compare-idx-empty');
  const toolbarReset = document.getElementById('compare-idx-reset');
  const activeFilters = document.getElementById('compare-idx-active-filters');
  const toolbar = document.querySelector('.terms-index-tools');
  const topBtn = document.getElementById('compare-idx-top');
  const flatBody = document.getElementById('compare-idx-flat-body');
  const COMPARE_INDEX_BASE = '/terms/compare/';
  ...
  function resolveEntryHref(href) {
    ...
    return `${COMPARE_INDEX_BASE}${String(href).replace(/^\.\//, '')}`;
  }
...
<td class="terms-idx-td-snippet compare-idx-td-summary" data-label="概要"${hrefAttr}>${summary ? highlightText(summary, query) : ''}</td>
  ...
  function bindRows() {
    ...
    flatBody.querySelectorAll('[data-entry-href]').forEach((cell) => {
      ...
      if (cell.classList.contains('compare-idx-td-title')) {
  ...
  function syncUrl() {
    ...
    urlSyncTimer = setTimeout(() => {
      ...
      const next = qs ? `${COMPARE_INDEX_BASE}?${qs}` : COMPARE_INDEX_BASE;
  ...
  document.getElementById('compare-idx-empty-reset')?.addEventListener('click', resetAll);
